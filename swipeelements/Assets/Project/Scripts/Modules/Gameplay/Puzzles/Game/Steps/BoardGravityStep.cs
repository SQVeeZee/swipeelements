...
{
    public class BoardGravityStep : MergesStep, ILockedStep
    {
        ...
        public static BoardGravityStep CalculateStep(MergesState state, HashSet<(int X, int Y)> ignored)
        {
            var step = new BoardGravityStep(state);
            step.ApplyGravity(ignored);
            return step;
        }

        private void ApplyGravity(HashSet<(int X, int Y)> ignored)
        {
            for (var x = 0; x < Final.Columns; x++)
            {
                for (var y = 1; y < Final.Rows; y++)
                {
                    ...
                    LockedCoords.Add(coord);
                    LockedCoords.Add(toCoord);
